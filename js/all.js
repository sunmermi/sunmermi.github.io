...
//   // 클래스 투글 함수 ( 어바웃버튼 클릭 오픈 또 클릭하면 클로즈 )
 ...
 (function(global){
  ...
  for(var i =0; i < about_btns_length; i++){
    ...
    about_btn.onclick = function(){
      // console.log(this);
      // console.log(this.num);
      hasClassRmove( about_content , 'slide-down');
      slideClass(about_content, this.num);
      return false;
    };
  ...
  // 요소에 클래스 있는지 없는지 확인후 있으면 삭제
  function hasClassRmove(arrNode, className){
    for(var i =0 , l = arrNode.length ; i < l ; i++){
      // console.log('ok');
      var aa = arrNode[i];
      // console.log(aa);
      var bb = aa.classList.contains(className);
      // console.log(bb);
      if(bb){
          // return console.log(aa);
        return aa.classList.remove(className);
      }
    }
  }
